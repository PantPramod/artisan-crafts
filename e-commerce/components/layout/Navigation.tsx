    ...
    return (
        <>
            <header className="flex items-center justify-between px-10 py-2 z-[999] sticky top-0 left-0 right-0 bg-white ">
                ...
                <nav className="flex gap-x-8  capitalize">
...

                {
                    <div
                        ...
                        className="w-[400px] bg-white fixed z-[9999] shadow-xl right-0 top-[83px] bottom-0 transition-all ease-in-out duration-300 translate-x-[100%] h-[calc(100vh-83px)] px-5 overflow-y-auto">
                        {cartData.length > 0 ? cartData.map((cart, index) => <div
                            key={cart._id}
                            className="flex items-center justify-between bg-white  border-b border-gray-300 py-5 overflow-y-auto ">
                            <div className="">
                        ...
                        <p className='text-2xl font-bold text-right py-5'>Subtotal: ₹{subtotal.toFixed(2)}</p>
                        {
                            cartData.length > 0 &&
                            <>
                                <Link href="/cart">
